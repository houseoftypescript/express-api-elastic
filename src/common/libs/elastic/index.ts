...
class ElasticSearchClient {
  ...
  public async addItemToIndex<T>(index: string, document: T): Promise<IndexResponse> {
  ...
  public async search(index: string, query: QueryDslQueryContainer): Promise<SearchResponse> {
    return this.client.search({ index, query });
  }
}

export const elasticSearchClient = new ElasticSearchClient({ node: ELASTICSEARCH_NODE });
